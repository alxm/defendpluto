...
void z_player_init(int8_t X, int8_t Y)
{
    ...
    z_player.lastShot = s_fps_getCounter();
}

void z_player_tick(void)
{
    if(s_button_pressed(z_controls.up) && z_player.y > 0) {
        z_player.y = (ZFix)(z_player.y - Z_FIX_ONE);
    } else if(s_button_pressed(z_controls.down) && z_fix_fixtoi(z_player.y) < S_HEIGHT - 1) {
        z_player.y = (ZFix)(z_player.y + Z_FIX_ONE);
    }

    if(s_button_pressed(z_controls.left) && z_player.x > 0) {
        z_player.x = (ZFix)(z_player.x - Z_FIX_ONE);
    } else if(s_button_pressed(z_controls.right) && z_fix_fixtoi(z_player.x) < S_WIDTH - 1) {
        z_player.x = (ZFix)(z_player.x + Z_FIX_ONE);
    }

    if(s_button_pressed(z_controls.a)) {
        if(s_fps_getCounter() - z_player.lastShot >= SHOOT_EVERY_N_FRAMES) {
        ...
        z_player.lastShot = s_fps_getCounter() - SHOOT_EVERY_N_FRAMES;
    }
}
